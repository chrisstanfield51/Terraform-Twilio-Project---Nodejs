terraform {
  ...
  backend "s3" {
    ...
    key    = "twilio-javascript/terraform.tfstate"
    region = "us-east-2"
  }
}
...
resource "aws_lambda_function" "lambda_code_deploy" {
  ...
  runtime = "nodejs14.x"
  handler = "TwilioFunction.handleTwilio"
  timeout = "60"
...
resource "null_resource" "twiliopython" {
  ...
  provisioner "local-exec" {
    command = "python3 updateTwilio.py '${aws_api_gateway_stage.gateway_stage.invoke_url}/message'"
  }
}
